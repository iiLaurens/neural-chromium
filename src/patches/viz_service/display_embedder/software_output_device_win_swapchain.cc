...
void SoftwareOutputDeviceWinSwapChain::EndPaintDelegated(
  ...
  // NEURAL CORTEX HOOK (SwapChain/DComp)
  if (cortex_writer_) {
      SkPixmap pixmap;
      if (sk_canvas_->peekPixels(&pixmap)) {
          cortex_writer_->Write(pixmap);
      } else {
          LOG(ERROR) << "[VisualCortex] SwapChain: PeekPixels Failed!";
      }
  } else {
      LOG(ERROR) << "[VisualCortex] SwapChain: No CortexWriter!";
  }
